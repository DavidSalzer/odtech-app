odtechApp.directive('takeVideo', ['camera', '$timeout', '$rootScope', '$filter', function (camera, $timeout, $rootScope, $filter) {
    return {
        restrict: 'E',
        ...
        link: function (scope, el, attrs) {
            ...
            scope.uploadVideo = function () {
                ...
                //    }
                //}, 30000);
                camera.editVideo(scope.video, "video", 1)
                            .then(function (data) {
                                var video = {videoCenter: {uri: data}};
                                camera.uploadPhoto(video, "video", 1);
                                ...
                                //    }
                                //}
                                ////if there is no data from server -save get the local video url
                                //if (!data || !data[0]) {
                                //    scope.results.answer = scope.videos;
                                //}
                                //scope.endMission(scope.results, scope.missionData);
                                ////   }

                            });
                //set the video with the local path
                scope.results.answer = scope.videos;
